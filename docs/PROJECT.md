...
> 版本 v1.4 · 2026-09-16 · 状态：M0–M4 完成 + 多简历管理 / 内置帮助 / 照片与社交账号 / 模板设计器（M5 基础版）
...
| v1.1 | 联系方式纸质化（不再渲染超链接，链接型展示为去协议域名、账号型为「平台 账号」）；社交字段扩充（微信/QQ/微博/小红书/Twitter/Instagram/Facebook/Telegram，JSON Resume 导入导出同步支持）；简历照片（样式面板上传、本地压缩为 JPEG dataURL 随简历存储，12 模板各设计照片位，未上传自动收起）；修复编辑器因高度约束错误导致最后一行无法滚动查看的问题 |
| v1.2 | 照片体验调整：改为原始分辨率存储（不压缩，超大照片上传前提示存储风险）；照片一律矩形呈现（贴合证件照比例，保持原始宽高比）；头版居中/左对齐的模板照片移至头部右侧空白处（绝对定位，不挤占标题排版），右侧已有内容的双栏/表格类模板照片与姓名并排 |
| v1.3 | 照片限制 ≤1MB（超限拒绝并提示）；照片显示大小可调（`--photo-scale` CSS 变量 + 样式面板滑杆 60%–160%，随简历保存，缩放不推挤正文）；修复 PDF 导出失败：打印时将 flex + 绝对定位的应用脚手架还原为文档流（此前绝对定位的预览容器参与分页被裁剪，导致 PDF 只有第一页或空白），页数徽标/空状态提示打印时隐藏；居中头版的姓名/联系方式不因照片预留空间而偏移（保持整页真居中） |
| v1.4 | **M5 基础版落地：模板设计器**。§8 规划的 Schema 化架构实现：`TemplateSchema`（JSON）+ `SchemaRenderer` 统一渲染器；模板面板新增「＋设计新模板」入口，全屏设计器提供页面/配色/字体/头部（排版、底色、照片位）/正文（分栏、标题风格、要点符号、技能标签化）与内容区块（顺序/显隐）的实时编辑；自定义模板保存至本地并出现在模板列表，支持 JSON 导出下载与上传导入（同 id 覆盖更新、内置 id 冲突自动重建）；拖拽自由编排与自定义模板分享仍为远期增强 |
| v1.5 | 按评审拆分定位：① 原 Schema 属性编辑下沉为样式面板的**专业调整**（页面边距覆盖、要点符号覆盖，经 CSS 变量作用于全部 12 款内置模板）；② **「＋」新建模板改为从零搭建的拖拽编排器**：素材库（头部/分隔线/各内容区块）拖入主栏/侧栏双区、块级拖拽排序与插入指示线、侧栏宽度可调，产出 Schema v2 块布局（`layout.main/side`），保存/导入导出与自定义模板体系统一；SchemaRenderer 支持块布局渲染（含 legacy v1 模式兼容） |
