...
| v1.3 | 照片限制 ≤1MB（超限拒绝并提示）；照片显示大小可调（`--photo-scale` CSS 变量 + 样式面板滑杆 60%–160%，随简历保存）；修复 PDF 导出失败：打印时将 flex + 绝对定位的应用脚手架还原为文档流（此前绝对定位的预览容器参与分页被裁剪，导致 PDF 只有第一页或空白），页数徽标/空状态提示打印时隐藏 |
