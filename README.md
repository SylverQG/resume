...
- **简历照片**：样式面板上传（≤1MB，原图本地存储），显示大小可调；12 个模板均设计照片位——头版居中/左对齐时位于头部右侧空白处，矩形呈现，未上传时自动收起
...
- [x] **模板设计器（基础版）**：「模板」面板 → ＋卡片进入全屏设计器，可视化调整配色/字体/头部/分栏/区块规则并实时预览，保存为自定义模板；支持 JSON 导出下载与上传导入（上传同 id 模板即完成更新）
- [ ] 拖拽自由编排的完整版设计器（区块级拖拽，基于现有 Schema 架构）
- [ ] 云同步 / 多设备（需最小后端）
- [ ] 在线简历分享页（需最小后端）
- [ ] AI 润色 / 翻译（需服务端代理）
